...
from models import Comment, Post, CommentLikes, User
...
def like_comment_controller(comment_id):
    user_id = int(get_jwt_identity())
    comment = db.session.get(Comment, comment_id)
    is_liked = CommentLikes.query.filter_by(user_id=user_id, comment_id=comment_id).first()
...
def unlike_comment_controller(comment_id):
    user_id = int(get_jwt_identity())
    comment = db.session.get(Comment, comment_id)
    like = CommentLikes.query.filter_by(user_id=user_id, comment_id=comment_id).first()
...
def get_comment_like_list_controller(comment_id):
    ...
    like_query = CommentLikes.query.filter_by(comment_id=comment_id).join(User)
